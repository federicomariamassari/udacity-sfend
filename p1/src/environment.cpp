...
void simpleHighway(pcl::visualization::PCLVisualizer::Ptr& viewer)
{
  ...
  // RENDER OPTIONS
  bool renderScene = false;  // false for point cloud rendering; true for LiDAR scans
  ...
  // Visualize the generated scan
  //renderRays(viewer, lidar->position, pointCloud);

  renderPointCloud(viewer, pointCloud, "pointCloud");

  // Instantiate point processor on the heap instead of on the stack (so use pointers)
  ...
  // Idea would be to separate kdtree and clustering, to be able to return the tree object and use the rendering 3d
  std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr> cloudClusters = pointProcessor->Clustering(segmentCloud.first, tree, 2.0, 3, 30);
