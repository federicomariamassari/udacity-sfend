...
/**
 * @brief Switch camera angle for rendered environment (custom addition) [1].
 * 
 * Resources:
 * 
 * [1] - https://github.com/udacity/SFND_Lidar_Obstacle_Detection/blob/master/src/environment.cpp#L66
 */
enum CameraAngle
{
  XY, Default
...
struct Car
{
  ...
  void render(pcl::visualization::PCLVisualizer::Ptr& viewer)
  {
    ...
    viewer->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_REPRESENTATION, 
      pcl::visualization::PCL_VISUALIZER_REPRESENTATION_WIREFRAME, name + "Topframe");
  }
  ...
  bool checkCollision(Vect3 point)
  {
    ...
    return (inbetween(xPrime, position.x, dimensions.x/2) && inbetween(yPrime, position.y, dimensions.y/2) && 
      inbetween(point.z, position.z + dimensions.z/3, dimensions.z/3)) || (inbetween(xPrime, position.x, 
        dimensions.x/4) && inbetween(yPrime, position.y, dimensions.y/2) && inbetween(point.z, position.z + 
        dimensions.z * 5/6, dimensions.z/6));
  }
};
