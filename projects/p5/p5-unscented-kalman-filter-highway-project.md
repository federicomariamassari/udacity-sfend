...
For each time step $k$, state prediction consists of three steps:
...
2. Predicting sigma points at time $k+1$ by propagating the augmented sigma points through the CTRV model.
3. Forecasting the _a priori_ state vector $x_{k+1|k}$ and covariance matrix $P_{k+1|k}$ with the sigma points predicted above.

These steps are encapsulated in [`UKF::Prediction()`](https://github.com/federicomariamassari/udacity-sfend/blob/main/projects/p5/src/ukf.cpp#L165-L175).
