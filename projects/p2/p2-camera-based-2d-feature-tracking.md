...
In this mid-term assignment, I implement a 2D feature-tracking algorithm to monitor objects in an image sequence from the KITTI dataset, using OpenCV. As frames are gradually loaded into a data buffer, I use state-of-the-art techniques to detect keypoints [Figure 1], compute their descriptors (vector summaries of landmarks' characteristics), match features between consecutive stills (using Brute Force or FLANN-based $k$-nearest neighbors matching), and finally assess the performance of each detector-descriptor combination in terms of speed and accuracy. This work will serve as the foundation for a camera-based estimate of time-to-collision (TTC) with the preceding vehicle, as part of next project: [Track an Object in 3D Space](https://github.com/federicomariamassari/udacity-sfend/blob/main/projects/p3/p3-track-an-object-in-3d-space.md).
...
Options can be set in the homonymous struct in the main file.
...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td><code>true</code> to write single frames in current working directory (if <code>bVis</code> is <code>true</code>)</td>
        </tr>
        <tr>
            <td><code>bSaveImagePairs</code></td>
            <td><code>true</code> to write image pairs with superimposed feature matches in current working directory (if <code>bVisMatches</code> is <code>true</code>)</td>
        </tr>
        <tr>
            <td><code>bLimitKpts</code></td>
            <td><code>true</code> to limit the number of keypoints to ~50 per frame (helpful for debugging and learning)</td>
        </tr>
    </tbody>
</table>
