[Home](../../README.md) | Previous: [Autonomous Systems Interview Practice](https://github.com/federicomariamassari/udacity-rsend/blob/main/projects/e1/e1-autonomous-systems-interview-practice.md) | Next: [Camera-Based 2D Feature Tracking](../p2/p2-camera-based-2d-feature-tracking.md)
...
The minimum bounding box algorithm performs relatively well even in more complex scenarios, such as tracking a cyclist in a highly non-linear environment [Figure 8]. The cyclist appears just north-east of the ego car, and is followed consistently throughout the route. Not all objects, however, are captured as effectively. For example, the poles in the bottom left corner of the figure occasionally display a "cross-shaped" pattern, a behaviour in which the point cloud is vertical but the bounding box is oriented horizontally. This tendency occurs because the ranges in the $X$ and $Y$ dimensions alternate in magnitude ($X > Y$, then $Y > X$), reversing the sorting of the singular vectors and causing flickering. This peculiarity suggests that a more statistically-sound measure than simple range may be needed to compare object dimensions.

__Figure 8: Tracking a Cyclist with PCA Bounding Boxes__
...
5. [Wikipedia: K-d tree](https://en.wikipedia.org/wiki/K-d_tree)
...
8. [Codex Technicanum: Find minumum oriented bounding box of point cloud (C++ and PCL)](http://codextechnicanum.blogspot.com/2015/04/find-minimum-oriented-bounding-box-of.html)
9. [GitHub: Frogee — Sorghum Reconstruction and Phenotyping, <code>boundingBox.h</code>](https://github.com/Frogee/SorghumReconstructionAndPhenotyping/blob/master/boundingBox.h)
