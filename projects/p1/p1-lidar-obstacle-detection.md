...
4. Once the singular vectors are sorted, ensure $V$ is still a valid rotation matrix. Among the other properties, it must satisfy $A*A^T = I(3)$ and $\det(A) = 1$. I find that the first one is generally preserved, but the second one is not, leading to a reflection matrix ($\det(A) = -1$). In which case, multiply the components of the least significant singular vector (which is not always the one associated to $z$!) by $-1$.
