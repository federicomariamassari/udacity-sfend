...
{ 
   ...
   * verticalAngle: The angle of direction between xy plane and ray (for example 0 radians is along xy plane and pi/2 radians is straight up)
  ...
  Ray(Vect3 setOrigin, double horizontalAngle, double verticalAngle, double setResolution)
    : origin(setOrigin), resolution(setResolution), direction(resolution * cos(verticalAngle) * cos(horizontalAngle), 
      resolution * cos(verticalAngle) * sin(horizontalAngle), resolution * sin(verticalAngle)), castPosition(origin), castDistance(0)
  {}
...
struct Lidar
{
    ...
    : cloud(new pcl::PointCloud<pcl::PointXYZ>()), position(0, 0, 2.6)
  {
    ...
    double horizontalAngleInc = pi/64;  // pi/128 for even higher resolution
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr scan()
  {
    ...
    cout << "Ray casting took " << elapsedTime.count() / 1000. << " milliseconds" << endl;
